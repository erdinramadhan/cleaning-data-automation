        ...
        for order_id, group_df in df.groupby("ID Pesanan"):
            try:
                ...
                for _, row in group_df.iterrows():
                    ...
                    # - Diskon Produk = diskon dari seller
                    # - Diskon Voucher = voucher yang lo setting di Desty (seller-funded juga)
                    # → Kedua-duanya SELLER. Desty gak punya platform subsidy.
                    item_seller_disc = (
                        abs(safe_int(row.get("Diskon Produk"), 0)) +
                        abs(safe_int(row.get("Diskon Voucher"), 0))
                    )
                    item_platform_disc = 0  # Desty gak ada platform subsidy
                ...
                header = OrderHeader(
                    ...
                    order_discount=0,  # Desty gak ada explicit order-level voucher
